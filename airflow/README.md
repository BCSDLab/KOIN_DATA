...
`insert_overwrite`의 교체 파티션 목록(`partitions`)에 함께 사용한다. 양 끝 날짜를
포함한 파티션을 통째로 교체하므로, 결과가 0건인 날짜도 기존 데이터를 비우고
범위 밖 파티션은 유지한다. 세션 계산을 위해 추가로 읽는 전날은 교체 대상이 아니다.
원천 테이블 미도착으로 결과가 비어도 같은 방식으로 비워지므로, 지정 범위의 원천
준비 상태를 확인한 뒤 실행한다.
